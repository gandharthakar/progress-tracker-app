...
const { updateUserGeneralSettingsFormValidationSchema } = require('../../libs/zod/schemas/userAreaValidationSchemas');
...
const generalSettingsController = async (req, res) => {
    ...
    try {
        ...
        if (token && user_email && user_full_name) {
            const valResult = updateUserGeneralSettingsFormValidationSchema.safeParse({
                email: user_email,
