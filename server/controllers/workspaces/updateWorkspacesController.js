...
const updateWorkspacesController = async (req, res) => {
    let status = 200;
    let response = {
        success: false,
        message: ""
            ...
            if (userAlreadyExist !== null) {
                if (workspaceIDCheck) {
                    ...
                    if (workspaceAlreadyExist !== null) {
                        await WorkspacesModel.findByIdAndUpdate({ _id: workspace_id }, { workspace_title, workspace_description });
                        status = 200;
                        response = {
                            success: true,
...
module.exports = updateWorkspacesController;